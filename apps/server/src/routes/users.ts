...
import { checkUserLimit } from "../lib/plan-limits.js";
...
const users = new Hono<Env>()
  ...
  .post(
    ...
    async (c) => {
      ...
      // ── Plan enforcement: user limit (hard block) ─────────────────
      const userCheck = await checkUserLimit(orgId);
      if (!userCheck.allowed) {
        return c.json({ error: userCheck.reason }, 403);
      }
